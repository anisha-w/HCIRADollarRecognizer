...
class DollarRecognizer{
    ...
    {
        if(gestureSetName=="gestureSetP1"){
            ...
            templates = new UnistrokeTemplate[gestureSetSize];
            String xmlDirPath = System.getProperty("user.dir");

		    //read folder name from config file instead of hardcoded.
            File xmlDir = new File(xmlDirPath + configDoc.getDocumentElement().getElementsByTagName("onlineRecognizerTemplatePath").item(0).getTextContent()); //ANIHSA : MAKE A MORE GENERIC PATH
        
            System.out.println("xmlDir.getAbsolutePath: " + xmlDir.getAbsolutePath());
            String userTemplateList[] = xmlDir.list(); 
            for(int j=0;j<userTemplateList.length;j++){  
                try{     
                    templates[j] = DollarRecognizerOffline.createGesture(new File(xmlDir.getAbsoluteFile()+"/"+userTemplateList[j])); //to store in a array list of arraylists
                }catch(Exception exp){
                    //
                }
            }
