...
Points are stored (line 61) on an ArrayList that grows dinamically depending on the user speed and stroke size.

## b) store a default set of templates to compare new candidate (input) gestures to 
### Method generateTemplates (line 370)
Populates the internal list of unistroke templates with preexisting 16 templates.
Templates are then processed with the $1 algorithm in (line 363) and processed points are stored for later.
...
### Method mouseReleased (line 72)
When the user finishes drawing the gesture, the method calls the points list processing (line 77) and recognition (line 79).
### Method processingGesture (line 119)
Orchestrator method that calls each one of the first three steps (resample,rotate,scale & translate) of the $1 recognizer sequentially.
Displays the points after each step (only for debugging. Currently commented)
### Method resample (line 137)
Resamples the X,Y point representation of the drawn gesture from the original count to a total of 64 points.
### Method rotateToZero (line 188)
Rotates the resampled gesture to a new angle using the gesture centroid as the pivot anchor.
### Method scaleToSquare (line 217)
Establishes the bounding box around the resampled and rotated gesture, and scales it to fit the margins.
### Method translateToOrigin (line 248)
Shifts all the the gesture points towards an origin position based on the centroid.
### Method recognize (line 262)
Compares the resampled, rotated, scaled and translated gesture to the processed templates, and returns the identified template and its score.
